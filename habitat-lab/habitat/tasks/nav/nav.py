...
@registry.register_measure
class NumSoftSuccess(Success):
    """
    見つけた数の割合をSuccessにすることで、数に対してSoftにした指標
    """

    cls_uuid: str = "numsoftsuccess"
    ...
    def update_metric(
        ...
        if (
            ...
            n_goal = len(self._sim.goals_dict)
            n_found = n_goal - len(self._sim.not_found_goals) + 1
            self._metric = n_found / n_goal
...
@registry.register_measure
class NumSoftSPL(SPL):
    def _get_uuid(self, *args: Any, **kwargs: Any) -> str:
        return "numsoftspl"

    def reset_metric(self, episode, task, *args: Any, **kwargs: Any):
        task.measurements.check_measure_dependencies(
            self.uuid, [DistanceToGoal.cls_uuid, NumSoftSuccess.cls_uuid]
        )
        ...
        self._start_position = self._sim.get_agent_state().position
        self._goal_ids = self._sim.not_found_goals
    ...
    def update_metric(
        ...
        # NumSoftSuccessを計算
        ep_num_soft_success = task.measurements.measures[NumSoftSuccess.cls_uuid].get_metric()
        ...
        if ep_num_soft_success == 0:
            self._metric = 0
        else:
            # 今まで到達したゴールへの最短経路を計算
            closest_goal_id, distance_to_closest_target = self._sim.closest_goal_id_and_dis()
            found_goal_ids = [
                goal_id for goal_id in self._goal_ids if not goal_id in self._sim.not_found_goals
            ]

            if (
                hasattr(task, "is_found_called")
                and task.is_found_called
                and distance_to_closest_target < 1.0
            ):
                found_goal_ids.append(closest_goal_id)

                
            found_goal_positions = [
                self._sim.goals_dict[goal_id] for goal_id in found_goal_ids
            ]
            geo_dis_to_found_goals = self._sim.calc_geo_dis_to_goals(self._start_position, found_goal_positions)
            distance_to_found_goals = {goal_id : geo_dis_to_found_goals[i] for i, goal_id in enumerate(found_goal_ids)}
            distance_from_found_goals = self._sim.calc_geo_dis_from_goals(found_goal_positions)
            distance_from_found_goals = {goal_id: distance_from_found_goals[i] for i, goal_id in enumerate(found_goal_ids)}
            best_found_goal_distance = np.inf
            for goal_id in found_goal_ids:
                d = distance_to_found_goals[goal_id] + distance_from_found_goals[goal_id]
                if d < best_found_goal_distance:
                    best_found_goal_distance = d
            
            self._metric = ep_num_soft_success * (
                best_found_goal_distance
                / max(
                    best_found_goal_distance, self._agent_episode_distance
                )
            )
