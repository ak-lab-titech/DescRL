...
class InstructionPredictor(nn.Module):
    ...
    def forward(self,
        ...
        batch_size = goal_belief.shape[0]     
        ...
        for i in range(self.max_instr_len-1):
            if i != self.max_instr_len-2:
                with torch.no_grad():
                    past_words = self.embed_word_tokens(past_tokens)
                    ...
                    logits = self.generator(
                        torch.cat([decoder_output, goal_belief], dim=1)
                    ) # (batch, 2506)
                ...
                instr_len, _, _ = decoder_output.shape
                expanded_belief = goal_belief.unsqueeze(0).repeat(instr_len, 1, 1)
                logits = self.generator(
                    torch.cat([decoder_output, expanded_belief], dim=2)
                ) # (instr_len, batch, vocab_size)
        return logits
    
    def embed_word_tokens(self, word_tokens):
        """
        word_tokens's shape must be (instr_len, batch)
        """
        instr_len, batch_size = word_tokens.shape
        word_tokens = word_tokens.reshape(-1) # (instr_len*batch, )
        words = self.word_emb(self.word_vocab_emb(word_tokens)) # (instr_len*batch, embed)
