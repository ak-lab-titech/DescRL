...
from soundspaces.tasks.nav import SpectrogramSensor, LocationBelief, KSAVENCategoryBelief, Category
...
class BeliefPredictor(nn.Module):
    def __init__(self, belief_config, device, input_size, pose_indices,
        ...
        print(f"belief predictor num_object: {self.num_objects}, num_regions: {self.num_regions}")
    ...
    def load_pretrained_weights(self):
        ...
        state_dict = torch.load(AUDIO_MODEL_PATH, map_location="cpu")
    ...
    def update(self, observations, dones):
        ...
        if self.predict_label:
            ...
            for i in range(batch_size):
                ...
                observations[KSAVENCategoryBelief.cls_uuid][i].copy_(torch.from_numpy(label_avg))
