...
class Sensor:
    ...
    def _get_audio_observation(self) -> Union[ndarray, "Tensor"]:
        ...
        # audio_sensor = self._agent._sensors["audio_sensor"] # changed
        audio_sensor = self._agent._sensors[self._spec.uuid]  # changed

        # tell the audio sensor about the agent location
        rot = self._agent.state.rotation
        angle = self.calc_angle_from_quaternion(rot)
        pos = self._agent._sensors[self._spec.uuid].node.absolute_translation # agent's position
        # agent_radius = 1.0
        agent_radius = 0.1

        eight_channel = False
        for i in range(len(self._sim.config.agents[0].sensor_specifications)):
            if self._sim.config.agents[0].sensor_specifications[i].__class__.__name__ == "AudioSensorSpec":
                eight_channel = (self._sim.config.agents[0].sensor_specifications[i].channelLayout.channelCount == 1)
        
        if eight_channel:
            j = int(self._spec.uuid[-1])
            # angle = ((angle + 45*j) % 360) * np.pi/180


            angle = ((angle + 90*j) % 360) * np.pi/180
        

            # rot = quat_from_angle_axis(angle, np.array([0, 1, 0]))
            pos = type(pos)([
                pos[0] + agent_radius * np.sin(angle - np.pi),
                pos[1],
                pos[2] + agent_radius * np.cos(angle - np.pi),
            ])


        audio_sensor.setAudioListenerTransform(
            pos,  # set the listener position
