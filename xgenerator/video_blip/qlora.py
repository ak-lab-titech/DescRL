...
import yaml
from transformers import Trainer, AutoProcessor, DataCollatorForSeq2Seq, Blip2Processor, TrainingArguments, BitsAndBytesConfig
import numpy as np
import torch
...
MODEL_NAME = "kpyu/video-blip-opt-2.7b-ego4d"
PROCESSOR = Blip2Processor.from_pretrained(MODEL_NAME)
# PROCESSOR.tokenizer.padding_side = "right"


def video_blip_collate_fn(batch):
    batch_size = len(batch)

    max_path_length = 0
    for x, _ in batch:
        if max_path_length < len(x):
            max_path_length = len(x)
    
    image_shape = np.shape(batch[0][0][0])
    batched_image_seqs = np.zeros(
        (batch_size, max_path_length) + image_shape,
        np.float32,
    )
    ...
    for i, (x, y) in enumerate(batch):
        batched_image_seqs[i, :len(x), :, :, :] = x
        path_masks[i, :len(x)] = False
        prompts.append(
            f"Question: what is the camera wearer doing? Answer: {y}"
        ) # TODO Should {y} include in prompts?
    
    batched_image_seqs = torch.from_numpy(np.array(batched_image_seqs))
    batched_image_seqs = batched_image_seqs.permute(0, 4, 1, 2, 3) # (b, c, t, h, w)

    inputs = process(PROCESSOR, video=batched_image_seqs, text=prompts)
    
    # save_dir = "data/video/finetune"
    # os.makedirs(save_dir, exist_ok=True)
    # video2 = batched_image_seqs.permute(2, 0, 3, 4, 1) / 255
    # generate_video(video2, f"{save_dir}/original.mp4")
    # video2 = inputs["pixel_values"].permute(2, 0, 3, 4, 1)
    # generate_video(video2, f"{save_dir}/processed.mp4")
...
class VideoBlipDataCollatorForVideoSeq2Seq(DataCollatorForSeq2Seq):
    ...
    def __call__(self, batch, return_tensors=None):
        inputs = video_blip_collate_fn(batch)
        return inputs
...
def main(config, logger):
    ...
    trainer = SFTTrainer(
        ...
        data_collator=VideoBlipDataCollatorForVideoSeq2Seq(
            PROCESSOR.tokenizer,
            max_instruction_length=config.train.max_instruction_length,
    ...
    for video, gt_instr in val_dataset:
        ...
        inputs = process(
            PROCESSOR,
            video=video,
            text=f"Question: what is the camera wearer doing? Answer:",
        )
        for k, v in inputs.items():
            inputs[k] = v.cuda()
        out = model.generate(**inputs)
        pred_instr = PROCESSOR.batch_decode(
...
if __name__=="__main__":
    ...
    with open("/home/4/ud02274/navigation/myss/xgenerator/video_blip/qlora_config.yaml", "r") as yml:
        config = yaml.safe_load(yml)
    
    config["save_path"] = f"data/models/video_blip/{args.model_name}"
